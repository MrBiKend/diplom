...
def add_student_window():
    def submit():
        ...
        messagebox.showinfo("Успех", "Группа обновлена успешно!")
        window.destroy()

    window = CTk()
    window.title("Добавить группу")

    first_name_label = CTkLabel(window, text="Номер группы")
    first_name_label.pack(pady=10)
    first_name_entry = CTkEntry(window)
    first_name_entry.pack(pady=10)

    last_name_label = CTkLabel(window, text="Специальность")
    last_name_label.pack(pady=10)
    last_name_entry = CTkEntry(window)
    last_name_entry.pack(pady=10)

    dob_label = CTkLabel(window, text="Количество людей")
    dob_label.pack(pady=10)
    dob_entry = CTkEntry(window)
    dob_entry.pack(pady=10)

    enrollment_label = CTkLabel(window, text="Доп. описание")
